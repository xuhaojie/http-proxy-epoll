...
static void *async_addr_info_main(void *arg) {
...
static void async_addr_info_del(struct asyncaddrinfo_resolution *res) {
	if (res->node) {
		free(res->node);
		res->node = NULL;
	}
...
void async_addr_info_init(size_t threads) {
	...
	for (size_t i = 0; i < asyncaddrinfo_num_threads; i++) {
		...
		assert(subfd >= 0);
		assert(!pthread_create(&asyncaddrinfo_threads[i], NULL, async_addr_info_main, (void*)(intptr_t)subfd));
	}
	assert(!close(fds[0]));
}

void async_addr_info_cleanup() {
	assert(asyncaddrinfo_threads);
